...
exports.obfuscateBundleInternals = obfuscateBundleInternals;
...
function obfuscateBundleInternals(code) {
    let counter = 0;
    const map = new Map();
    const nameFor = (key) => {
    ...
    const collect = (regex) => {
        for (let m = regex.exec(code); m; m = regex.exec(code))
            modulePaths.add(m[2]);
    };
    collect(/require\(\s*(["'])([^"'\n]+?)\1\s*\)/g);
...
//# sourceMappingURL=obfuscate-internals.js.map