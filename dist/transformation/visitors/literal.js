...
const transformNumericLiteralExpression = (expression, context) => {
    ...
    const sourceText = expression.pos >= 0 ? expression.getText() : expression.text;
    const isFloat = /[.eE]/.test(sourceText);
    return lua.createNumericLiteral(Number(expression.text), expression, isFloat);
};
